...
typedef LPVOID WINAPI (*GetProcAddr)(LPVOID module,const char * name);
typedef LPVOID WINAPI (*Load_DLL)(const char * name);
	...
	Load_Import:
		...
		while(imp->Name)
		{
			...
			while(Othunk->u1.AddressOfData)
			{
				...
				{
					*(PULONG)Fthunk=(ULONG)pe->Get_Proc(dll,(LPSTR)IMAGE_ORDINAL(Othunk->u1.Ordinal));
				}
				else
				{
					PIMAGE_IMPORT_BY_NAME fn=(PIMAGE_IMPORT_BY_NAME)(base+Othunk->u1.AddressOfData);
					*(PULONG)Fthunk=(ULONG)pe->Get_Proc(dll,fn->Name);
				}
				Othunk++;
				Fthunk++;
			}
...
int main(int i,char *arg[])
{
	...
	{
		pe.Brloc=TRUE;printf("[!]Failed To Allocate Memory At Prefered Memory Address.Trying Alernative..\n");
	...
	pe.base=Rbase;
	pe.Get_Proc=(LPVOID)GetProcAddress;
	pe.Load_Dll=(LPVOID)LoadLibraryA;
	...
	DWORD len=(DWORD)((ULONG)main-(ULONG)Adjust_PE);
